...
file_dir <- "/mnt/g/GOSAT_SIF/original"
...
add_pa <- function(gosat_file){
  
  t_start <- Sys.time()
  
  message(paste0("Adding variables to: ", gosat_file))
  
  myfile <- nc_open(gosat_file)
  ...
  # Write to file
  myfile   <- nc_open(gosat_file, write = TRUE)
  ...
  message(paste0("Done with ", basename(gosat_file), ". ", (t_end - t_start)))
