...
add_pa <- function(gome2_file){
  
  t_start <- Sys.time()
  
  message(paste0("Adding variables to: ", gome2_file))
  
  myfile <- nc_open(gome2_file)
  ...
  # Write to file
  myfile   <- nc_open(gome2_file, write = TRUE)
  ...
  message(paste0("Done with ", basename(gome2_file), ". ", (t_end - t_start)))
